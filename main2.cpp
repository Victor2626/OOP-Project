...
public:
    ...
    void set_durata(int new_durata);
};
...
class Magazin:public Produs{
    ...
    int perioada_produs;
public:
    Magazin(int nr_cal_product=190, int perioada_product=3): nr_cal_produs(nr_cal_product), perioada_produs(perioada_product){}
...
void Magazin::prezentare() const {
    if(calorii())cout<<"Nesanatos"<<endl;
    else cout<<"Sanatos"<<endl;
    if(expirare())cout<<"Expirat"<<endl;
    else cout<<"Valid"<<endl;
}
bool Magazin::calorii() const {
    if(nr_cal_produs>=300)return true;
    else return false;
}
bool Magazin::expirare() const {
    if(perioada_produs>=7)return true;
    else return false;
}
ostream& operator<<(ostream& out, const Magazin& magazin) { //operatorul de afisare
    out<<"Numar de calorii ale produsului: "<<magazin.nr_cal_produs<<endl;
    out<<"Perioada de depozitare a produsului: "<<magazin.perioada_produs<<endl;
    return out;
}
...
protected:
    int lungime_drum;
    int nr_vizitatori_drum;
public:
    Drum(int length_road=20, int nr_visitors_road=40):lungime_drum(length_road), nr_vizitatori_drum(nr_visitors_road){}
    Drum(const Drum& other){
        lungime_drum=other.lungime_drum;
        nr_vizitatori_drum=other.nr_vizitatori_drum;
    }
    ...
    virtual void lungime() const = 0;
    virtual void circulatie() const = 0;
    friend ostream& operator<<(ostream& out, const Drum& drum);
};
ostream& operator<<(ostream& out, const Drum& drum) { //operatorul de afisare
    out<<"Lungimea drumului: "<<drum.lungime_drum<<endl;
    out<<"Numarul de oameni care circula: "<<drum.nr_vizitatori_drum<<endl;
    return out;
}
class Asfalt:public Drum{
public:
    Asfalt(int length_road=15, int nr_visitors_road=56): Drum(length_road, nr_visitors_road){}
    ...
    void lungime() const override;
    void circulatie() const override;
    friend ostream& operator<<(ostream& out, const Asfalt& asfalt);
};
void Asfalt::lungime() const {
    if(lungime_drum>=50)cout<<"Lung";
    else cout<<"Scurt";
}
void Asfalt::circulatie() const {
    if(nr_vizitatori_drum>=20)cout<<"Populat";
    else cout<<"Nepopulat";
}
ostream& operator<<(ostream& out, const Asfalt& asfalt) { //operatorul de afisare
    out<<"Lungimea drumului de asfalt: "<<asfalt.lungime_drum<<endl;
    out<<"Numarul de oameni care circula pe drumul cu asfalt: "<<asfalt.nr_vizitatori_drum<<endl;
    return out;
}
class Pietris:public Drum{
public:
    Pietris(int length_road=15, int nr_visitors_road=56): Drum(length_road, nr_visitors_road){}
...
void Pietris::lungime() const {
    if(lungime_drum>=50)cout<<"Lung";
    else cout<<"Scurt";
}
void Pietris::circulatie() const {
    if(nr_vizitatori_drum>=20)cout<<"Populat";
    else cout<<"Nepopulat";
}
ostream& operator<<(ostream& out, const Pietris& pietris) { //operatorul de afisare
    out<<"Lungimea drumului de petris: "<<pietris.lungime_drum<<endl;
    out<<"Numarul de oameni care circula pe drumul cu pietris: "<<pietris.nr_vizitatori_drum<<endl;
    return out;
}
...
public:
    ZooException(const string& message, bool isFeedingTime):message(message), isFeedingTime(isFeedingTime){}
...
int main() {
    ...
    cout<<magazin1;
    cout<<"\n";
    Magazin magazin2; //instantele de upcasting
    Produs& produsRef=magazin2;
    produsRef.calorii();
    produsRef.expirare();
    Drum* drum1 = new Asfalt(); //polimorfism la executie
    Drum* drum2 = new Pietris();
    cout<<"Detalii drum de asfalt:"<<endl;
    cout<<*drum1;
    drum1->lungime();
    cout<<endl;
    drum1->circulatie();
    cout<<endl;
    cout<<endl;
    cout<<"Detalii drum de pietris:"<<endl;
    cout<<*drum2;
    drum2->lungime();
    cout<<endl;
    drum2->circulatie();
